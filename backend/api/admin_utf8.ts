...
// APPLY GLOBAL ADMIN SECURITY
router.use(authGuard);
router.use(roleGuard('admin'));
router.use(rateLimitGuard(60000, 50)); // High limit for admins but still throttled

/**
 * @route   GET /api/admin/users
 * @desc    Get all users with merged profile/VIP data
 * @access  Admin Only (RBAC Enforced)
 */
router.get('/users', async (req, res) => {
    try {
        // Fetch all auth users via Admin SDK
